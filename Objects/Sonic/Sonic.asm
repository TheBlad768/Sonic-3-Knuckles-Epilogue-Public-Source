...
Sonic_Restart:
		...
		subq.w	#1,$3E(a0)
		bne.s	locret_1258E
		cmpi.b	#id_OptionsScreen,(Game_mode).w
		beq.s	Sonic_Restart_SetRestart
		tst.b	(QuickStart_mode).w
		bne.s	Sonic_QuickStart

Sonic_Restart_SetRestart:
		tst.b	(Time_over_flag).w
		beq.s	Sonic_Restart_SetRestart_NoTimeOver
		move.b	#id_OptionsScreen,(Game_mode).w	; set Game Mode
		rts
; ---------------------------------------------------------------------------

Sonic_Restart_SetRestart_NoTimeOver:
...
Sonic_QuickStart:
;		cmpi.b	#id_OptionsScreen,(Game_mode).w
;		bne.s	Sonic_QuickStartNotOptions
;		move.l	#Obj_SonicRocket_Restore,(v_Breathing_bubbles).w
;		bra.s	Sonic_QuickStart_SetCamera
; ---------------------------------------------------------------------------

;Sonic_QuickStartNotOptions:
		tst.w	(V_scroll_speed).w
		beq.s	Sonic_QuickStart_Normal
		move.b	#$60,(Boss_Events).w
		move.l	#Obj_SonicFlying_Control,(v_Breathing_bubbles).w

Sonic_QuickStart_SetCamera:
		...
		bra.s	Sonic_QuickStart_Restore
; ---------------------------------------------------------------------------

Sonic_QuickStart_Normal:
		moveq	#0,d0
		...
		move.l	d0,x_vel(a0)
		move.w	d0,ground_vel(a0)
		move.b	#id_SonicHurt,routine(a0)	; Hit animation
		move.b	#id_Hurt,anim(a0)		; Set falling animation
		bclr	#Status_OnObj,status(a0)
		bclr	#Status_Push,status(a0)		; Player is not standing on/pushing an object
		bset	#Status_InAir,status(a0)
		move.w	(Camera_X_pos).w,d0
		addi.w	#$A0,d0
		move.w	d0,x_pos(a0)
...
Sonic_QuickStart_Restore:
		...
		cmpi.w	#99,(Death_Count).w
		bhs.s	Sonic_QuickStart_Death
		addq.w	#1,(Death_count).w
		ori.b	#1,(Update_death_count).w

Sonic_QuickStart_Return:
		rts
; ---------------------------------------------------------------------------

Sonic_QuickStart_Death:
		move.b	#id_DeathScreen,(Game_mode).w
		rts
