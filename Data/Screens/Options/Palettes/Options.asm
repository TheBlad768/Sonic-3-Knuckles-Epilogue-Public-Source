...
; RAM
vOptions_Timer:				= Object_load_addr_front		; word
vOptions_TextFactor:			= Object_load_addr_front+2	; byte
vOptions_End:				= Object_load_addr_front+3	; byte
vOptions_Music_test_count:	= Object_load_addr_front+4	; word
vOptions_Sound_test_count:	= Object_load_addr_front+6	; word
vOptions_PlayFlag:			= Object_load_addr_front+8	; byte
vOptions_Count:				= Object_load_addr_front+$A	; word
vOptions_Frame_Counter:		= Object_load_addr_front+$C	; word
...
Options_PressStart:
		...
		beq.w	Options_PressStart_LoadMiniGame
		disableInts
		stopZ80
		...
		clr.w	(Object_load_addr_front+8).w
		...
		tst.b	(FirsStart_Flag).w
		beq.s	+
		lea	Options_PressStart_LoadLevel(pc),a1
		bmi.s	+
		lea	Transition_Screen_Restart(pc),a1
...
Options_PressStart_Load:
		...
		bne.s	Options_PressStart_LoadLevel
		clr.w	(Object_load_addr_front+$A).w
		jmp	(a1)
; ---------------------------------------------------------------------------

Options_PressStart_LoadLevel:
...
OptionsScreen_Process_LoadText:
		...
		bsr.w	SmoothDrawArtText2
		move.l	#vOptions_Buffer>>1,d1
...
OptionsScreen_Process_End:
		tst.b	(QuickStart_mode).w
		bne.s	OptionsScreen_Process_SetEnd
		cmpi.w	#5,(vOptions_Count).w
		beq.s	OptionsScreen_Process_SetEnd
		cmpi.w	#4,(vOptions_Count).w
		beq.s	OptionsScreen_Process_SetEnd
		cmpi.w	#6,(vOptions_Count).w
		beq.s	OptionsScreen_Process_HScroll
		tst.b	(FirsStart_Flag).w
		bmi.s	OptionsScreen_Process_SetEnd

OptionsScreen_Process_HScroll:
		addi.w	#$80,(HScroll_Shift).w
		cmpi.w	#$1000,(HScroll_Shift).w
...
-		move.w	(a1)+,d0
		...
		add.w	d3,d0
		move.w	d0,VDP_data_port-VDP_data_port(a6)
+		bsr.s	Options_LoadFlashing
		bsr.s	Options_LoadQuickStart
		bsr.w	Options_DrawMusicNumber
