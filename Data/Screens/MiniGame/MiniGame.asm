...
vMiniGame_Cheat_flag:		= Boss_Events+4				; byte
vMiniGame_Cheat2_flag:		= Boss_Events+5				; byte
vMiniGame_Cheat3_flag:		= Boss_Events+6				; byte
vMiniGame_Cheat4_flag:		= Boss_Events+7				; byte
...
MiniGame_Screen:
		...
		ResetDMAQueue
		lea	PLC_MiniGameTitle(pc),a5
		jsr	(LoadPLC_Raw_KosM).w
		lea	(MapEni_MiniGameTitleFG).l,a0
		lea	(RAM_start).l,a1
		move.w	#$4001,d0
		jsr	(Eni_Decomp).w
		copyTilemap3		vram_fg, 256, 224
...
MiniGame_LoadGame:
		...
		jsr	(Clear_DisplayData).w
		lea	(VDP_control_port).l,a6
		move.w	#$8004,VDP_control_port-VDP_control_port(a6)
		move.w	#$8A00+255,(H_int_counter_command).w
		move.l	#VInt,(V_int_addr).w
		move.l	#HInt,(H_int_addr).w
		lea	PLC_MiniGame(pc),a5
		jsr	(LoadPLC_Raw_KosM).w
...
MiniGame_Pause:
		...
		bpl.s	MiniGame_Pause_NoPause			; if not, branch
+		tst.b	(GoodMode_flag).w
		beq.s	MiniGame_Pause_NoPause
		st	(Game_paused).w
		SMPS_PauseMusic
...
MiniGame_Pause_Loop:
	...
	if GameDebug=1
		...
		bne.s	MiniGame_Pause_FrameAdvance		; if yes, branch
		btst	#bitC,(Ctrl_1_pressed).w					; is button C pressed?
		bne.s	MiniGame_Pause_FrameAdvance		; if yes, branch
MiniGame_Pause_ChkStart:
	endif
		tst.b	(Ctrl_1_pressed).w						; is Start pressed?
		bpl.s	MiniGame_Pause_Loop				; if not, branch

MiniGame_Pause_ResumeMusic:
		SMPS_UnpauseMusic

MiniGame_Pause_Unpause:
		clr.b	(Game_paused).w

MiniGame_Pause_NoPause:
		rts
; ---------------------------------------------------------------------------
	if GameDebug=1
MiniGame_Pause_FrameAdvance:
		st	(Game_paused).w
		SMPS_UnpauseMusic
		rts
	endif
; End of function MiniGame_Pause_Game
; ---------------------------------------------------------------------------
; Load Level
; ---------------------------------------------------------------------------
...
.NoCheat3:
		...
		move.w	#$8700+(3<<4),VDP_control_port-VDP_control_port(a6)
		moveq	#bgm_GHZSMS,d0
...
MiniGame_HUDScore:
		...
		move.l	(vMiniGame_Score).w,d0
		cmp.l	(MiniGame_SaveScore).w,d0
		blt.s		MiniGame_HUDScore_DrawSaveScore
		move.l	d0,(MiniGame_SaveScore).w

MiniGame_HUDScore_DrawSaveScore:
		move.l	(MiniGame_SaveScore).w,d0
...
MiniGame_Code_Process:
		...
		lea	MiniGame_CodeDat_Index(pc),a4
		lea	MiniGame_CheatCode_Index(pc),a5
		moveq	#4-1,d7
...
MiniGame_CheatCode_Index:
		offsetEntry.w MiniGame_SetFirstCheatCode		; 0
		offsetEntry.w MiniGame_SetSecondCheatCode		; 2
		offsetEntry.w MiniGame_SetThirdCheatCode		; 4
		offsetEntry.w MiniGame_SetFourthCheatCode		; 6
; ---------------------------------------------------------------------------
...
MiniGame_SetSecondCheatCode:
		moveq	#signextendB(bgm_Fade),d0
		eori.b	#-1,(vMiniGame_Cheat2_flag).w
		bne.s	+
		moveq	#signextendB(bgm_TitleSMS),d0
+		move.b	d0,(Clone_Driver_RAM+SMPS_RAM.variables.queue.v_playsnd1).w	; play music
		rts
; ---------------------------------------------------------------------------
...
MiniGame_Code:
		...
		andi.b	#$7F,d0
		beq.s	MiniGame_Code_Return
		move.b	(Ctrl_1).w,d0
		cmp.b	(a1)+,d0
		bne.s	MiniGame_Code_Fail
		addq.b	#1,(a2)
		tst.b	(a1)
		bne.s	MiniGame_Code_Return
		sfx	sfx_Ring,0,0,0
		pea	(a3)
...
MiniGame_Touch_EnemyNormal:
		...
		bra.w	MiniGame_Touch_ChkHurt_Return
...
.Sync:
		lea	(VDP_data_port).l,a6
		locVRAM	tiles_to_bytes($FA),VDP_control_port-VDP_data_port(a6)
		moveq	#0,d0
...
; Level 2	; $0E
		offsetEntry.w PLC_MiniGameDEZ2
		dc.l MapEni_MiniGameDEZBG
		dc.w $4058
...
PLC_MiniGameTitle:
		...
		plreq 1, ArtKosM_MiniGameTitleFG
PLC_MiniGameTitle_End

PLC_MiniGameDEZ:
		dc.w ((PLC_MiniGameDEZ_End-PLC_MiniGameDEZ)/6)-1
		plreq 1, ArtKosM_MiniGameDEZFG
		plreq $6C, ArtKosM_MiniGameDEZBG
PLC_MiniGameDEZ_End

PLC_MiniGameDEZ2:
		dc.w ((PLC_MiniGameDEZ2_End-PLC_MiniGameDEZ2)/6)-1
		plreq 1, ArtKosM_MiniGameDEZFG2
		plreq $58, ArtKosM_MiniGameDEZBG
PLC_MiniGameDEZ2_End

PLC_MiniGameGHZ:
		dc.w ((PLC_MiniGameGHZ_End-PLC_MiniGameGHZ)/6)-1
		plreq 1, ArtKosM_MiniGameGHZFG
		plreq $31, ArtKosM_MiniGameGHZBG
PLC_MiniGameGHZ_End

PLC_MiniGame:
		dc.w ((PLC_MiniGame_End-PLC_MiniGame)/6)-1
		plreq $D0, ArtKosM_MiniGameText
		plreq $100, ArtKosM_MiniGameMiniSonic
		plreq $140, ArtKosM_MiniGameMiniEnemy
PLC_MiniGame_End

MiniGame_CodeDat_Index:
		offsetEntry.w MiniGame_CodeDat
		offsetEntry.w MiniGame_Code2Dat
		offsetEntry.w MiniGame_Code3Dat
		offsetEntry.w MiniGame_Code4Dat
; ---------------------------------------------------------------------------
