...
vMiniGame_Cheat_flag:		= Object_load_addr_front+5	; byte
...
vMiniGame_SaveScore:		= Object_load_addr_front+$C	; long
...
MiniGame_LoadLevelData:
		moveq	#0,d0					; DEZ(Destroyed)



;		moveq	#1*$E,d0				; DEZ(Normal)
		...
		tst.b	(vMiniGame_Cheat_flag).w
		beq.s	.NoCheat
		moveq	#2*$E,d0				; GHZ(Normal)
		move.w	#$8700+(3<<4),VDP_control_port-VDP_control_port(a5)
		moveq	#bgm_GHZSMS,d1

.NoCheat:
		move.b  d1,(Clone_Driver_RAM+SMPS_RAM.variables.queue.v_playsnd1).w
		adda.w	d0,a4
...
MiniGame_HUDScore:
		...
		lea	(ArtUnc_TerminalText).l,a1
		locVRAM	tiles_to_bytes($400),d0
		move.l	(vMiniGame_Score).w,d1

MiniGame_HudLives_Loop:
		moveq	#0,d2
		move.l	d0,VDP_control_port-VDP_data_port(a6)
		move.l	(a2)+,d3

MiniGame_HudLives_FindDigit:
		sub.l	d3,d1
		bcs.s	MiniGame_HudLives_LivesWrite
		addq.w	#1,d2
		bra.s	MiniGame_HudLives_FindDigit
; ---------------------------------------------------------------------------

MiniGame_HudLives_LivesWrite:
		add.l	d3,d1
		lsl.w	#5,d2
		lea	(a1,d2.w),a3
	rept 8
...
MiniGame_HudLives_LivesNext:
		addi.l	#$200000,d0
		dbf	d6,MiniGame_HudLives_Loop
		rts
; ---------------------------------------------------------------------------
; Cheat Code
; ---------------------------------------------------------------------------
...
MiniGame_Code:
		...
		cmp.b	(a1),d0
		bne.s	+
		addq.w	#1,(vMiniGame_DCount).w
		tst.b	1(a1)
		bne.s	++
		sfx	sfx_Ring,0,0,0
		st	(vMiniGame_Cheat_flag).w
+		clr.w	(vMiniGame_DCount).w
+		rts
; ---------------------------------------------------------------------------
; Resize
; ---------------------------------------------------------------------------
...
;		move.w	#1*60,(vMiniGame_Timer).w
		...
		bne.s	Load_MiniEnemy_Return
		move.b	#((Load_MiniEnemy_DataIndex2-Load_MiniEnemy_DataIndex)/2)-1,(vMiniGame_Level).w
...
obSM_Timer				= $2E	; .w
obSM_Hitcount			= $30	; .b
obSM_Invultimer			= $31	; .b

; status_secondary
Status_SM_Attack		= $00	; bit
Status_SM_Jump			= $01	; bit
Status_SM_Hurt			= $02	; bit
Status_SM_Death			= $03	; bit
...
MiniSonic_Main_Display:
		...
		move.b	obSM_Invultimer(a0),d0
		beq.s	MiniSonic_Main_Touch
		subq.b	#1,obSM_Invultimer(a0)
		lsr.b	#3,d0
...
MiniSonic_Attack:
		...
		bset	#Status_SM_Attack,status_secondary(a0)
		sfx	sfx_AttackSMS,0,0,0
		move.w	#1*10,obSM_Timer(a0)
		clr.w	x_vel(a0)
		rts
; ---------------------------------------------------------------------------

MiniSonic_Attack_Clear:
		subq.w	#1,obSM_Timer(a0)
		bpl.s	MiniSonic_Attack_Return
		bclr	#Status_SM_Attack,status_secondary(a0)
...
MiniSonic_Jump:
		...
		bset	#Status_SM_Jump,status_secondary(a0)
...
MiniSonic_Jump_Fall:
		...
		move.w	d0,y_pos(a0)
		bclr	#Status_SM_Jump,status_secondary(a0)
		clr.w	y_vel(a0)
...
MiniSonic_Hurt:
		...
		move.w	d0,y_pos(a0)
		bclr	#Status_SM_Hurt,status_secondary(a0)
		clr.l	x_vel(a0)
...
MiniHud_Main:
		move.b	(Player_1+obSM_Hitcount).w,d0
		andi.b	#3,d0
...
MiniGame_Touch_ChkHurt:
		...
		blo.s		+
		neg.w	x_vel(a0)
+		move.b	#3*60,obSM_Invultimer(a0)
		bset	#Status_SM_Hurt,status_secondary(a0)
		subq.b	#1,obSM_Hitcount(a0)
		bne.s	MiniGame_Touch_ChkHurt_Return
		bset	#Status_SM_Death,status_secondary(a0)
		clr.b	obSM_Invultimer(a0)
