...
-		move.l	(a1)+,(a2)+
		...
		move.w	#0,(vMiniGame_CamMin).w
		move.w	#0,(vMiniGame_CamMax).w
...
-		move.b	#VintID_Main,(V_int_routine).w
		jsr	(DelayProgram).l
		move.w	(vMiniGame_CamVel).w,(vMiniGame_CamMin).w
		bsr.w	MiniGame_Deform
		disableInts
		bsr.w	MiniGame_SynchroAnimate
		enableInts
		bsr.s	MiniGame_Resize
		bsr.w	Process_Sprites
		bsr.w	Render_Sprites
		...
		moveq	#0,d2
		move.w	(vMiniGame_CamVel).w,d2
		andi.w	#$1FE,d2
		lsr.w	#3,d2
		swap	d2
		add.l	d2,d1
		move.l	d1,($FFFF6000).l
...
MiniGame_Resize:
		...
		move.w	MiniGame_Resize_Index(pc,d0.w),d0
		jmp	MiniGame_Resize_Index(pc,d0.w)
; ---------------------------------------------------------------------------

MiniGame_Resize_Index: offsetTable
		offsetTableEntry.w MiniGame_Resize_Start		; 0
		offsetTableEntry.w MiniGame_Resize_Check		; 2
; ---------------------------------------------------------------------------

MiniGame_Resize_Start:
		move.w	(vMiniGame_CamMax).w,d1
		cmp.w	(vMiniGame_CamVel).w,d1
		bne.s	MiniGame_Resize_Return
		subq.w	#1,(vMiniGame_Timer).w
		bpl.s	MiniGame_Resize_Return
		addq.b	#2,(Dynamic_resize_routine).w
		bra.w	Load_MiniEnemy
; ---------------------------------------------------------------------------

MiniGame_Resize_Check:
		tst.b	(vMiniGame_Count).w
		bne.s	MiniGame_Resize_Return
		subq.b	#2,(Dynamic_resize_routine).w
		addi.w	#$20,(vMiniGame_CamMax).w

; Reset Camera
		move.w	(Player_1+$32).w,d1
		sub.w	(vMiniGame_CamVel).w,d1
		blt.s		MiniGame_Resize_Return
		move.w	(vMiniGame_CamVel).w,d1
		add.w	x_pos(a0),d1
		move.w	d1,(vMiniGame_CamVel).w
		move.w	d1,(Player_1+$32).w



		add.w	d1,(vMiniGame_CamMax).w
...
MiniSonic_Walk_SetVel:
		...
		move.w	$32(a0),d1
		sub.w	(vMiniGame_CamVel).w,d1
		blt.s		MiniSonic_Walk_CamLeft
		bge.s	MiniSonic_Walk_CamRight
		rts
; ---------------------------------------------------------------------------

MiniSonic_Walk_CamLeft:
		add.w	(vMiniGame_CamVel).w,d1
		cmp.w	(vMiniGame_CamMin).w,d1
		bgt.s	MiniSonic_Walk_SetVel_SetCamPos
		move.w	(vMiniGame_CamMin).w,d1

;		cmp.w	(vMiniGame_CamMax).w,d1
;		beq.s	MiniSonic_Walk_SetVel_SetPos

		cmpi.w	#$100,x_pos(a0)
		bgt.s	MiniSonic_Walk_SetVel_ResetPos

MiniSonic_Walk_SetVel_SetPos:
		add.l	d0,x_pos(a0)
		rts
; ---------------------------------------------------------------------------

MiniSonic_Walk_SetVel_ResetPos:
		move.w	#$100,x_pos(a0)

MiniSonic_Walk_SetVel_SetCamPos:
		move.w	d1,(vMiniGame_CamVel).w
		add.l	d0,$32(a0)
		rts
; ---------------------------------------------------------------------------

MiniSonic_Walk_CamRight:
		add.w	(vMiniGame_CamVel).w,d1
		cmp.w	(vMiniGame_CamMax).w,d1
		blt.s		MiniSonic_Walk_SetVel_SetCamPos
		move.w	(vMiniGame_CamMax).w,d1
...
;		beq.s	MiniSonic_Walk_SetVel_SetPos
		...
		cmpi.w	#$100,x_pos(a0)
		blt.s		MiniSonic_Walk_SetVel_ResetPos
		bra.s	MiniSonic_Walk_SetVel_SetPos
; ---------------------------------------------------------------------------
