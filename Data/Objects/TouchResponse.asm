...
loc_10364:
		bra.s	loc_1036E
; ---------------------------------------------------------------------------
; Float hurting Sonic/Tails/Knuckles subroutine
; ---------------------------------------------------------------------------
...
HurtCharacter_Float:
		tst.w	d0									; does Sonic have any rings?
		beq.s	Kill_Character						; if not, branch
		bsr.w	Create_New_Sprite
		bne.s	HurtCharacter_Float_Set
		move.l	#Obj_Bouncing_Ring,address(a1)		; load bouncing multi rings object
		move.w	x_pos(a0),x_pos(a1)
		move.w	y_pos(a0),y_pos(a1)
		move.w	a0,$3E(a1)

HurtCharacter_Float_Set:
		move.w	#-$200,x_vel(a0)
		move.w	x_pos(a0),d0
		cmp.w	x_pos(a2),d0
		blo.s		.isleft
...
.isleft:
		clr.w	y_vel(a0)
		clr.w	ground_vel(a0)
		move.b	#id_Float2,anim(a0)
		move.b	#2*60,invulnerability_timer(a0)			; set temp invincible time to 2 seconds

HurtCharacter_Float_End:
		moveq	#-1,d0
		rts
; ---------------------------------------------------------------------------
