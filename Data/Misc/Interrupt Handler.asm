...
VInt_Level_Cont:
		tst.b	(Boss_flag).w
		beq.s	+
		dma68kToVDP Hud_Boss_ArtBuffer,(ArtTile_HUDBoss*$20),tiles_to_bytes(8),VRAM
+		dma68kToVDP H_scroll_buffer,vram_hscroll,$380,VRAM
		dma68kToVDP Sprite_table_buffer,vram_sprites,$280,VRAM
		jsr	(Process_DMA_Queue).w
...
VInt_SpecialFunction:
		...
		beq.s	VIntSpecial_Function_Return
		move.w	VIntSpecial_Index-2(pc,d0.w),d0
		jmp	VIntSpecial_Index(pc,d0.w)
; ---------------------------------------------------------------------------

VIntSpecial_Index: offsetTable
		offsetTableEntry.w VIntSpecial_VScrollFG		; 2
		offsetTableEntry.w VIntSpecial_VScrollFull		; 4
; ---------------------------------------------------------------------------

VIntSpecial_VScrollFG:
		lea	(VDP_data_port).l,a6
		move.l	#vdpComm($0000,VSRAM,WRITE),VDP_control_port-VDP_data_port(a6)
		move.w	(V_scroll_value).w,d0
		add.w	(V_scroll_shift).w,d0
		move.w	d0,VDP_data_port-VDP_data_port(a6)

VIntSpecial_Function_Return:
		rts
; ---------------------------------------------------------------------------

VIntSpecial_VScrollFull:
		stopZ80
		dma68kToVDP V_scroll_buffer,$0000,$50,VSRAM
		startZ80
		rts
; ---------------------------------------------------------------------------
