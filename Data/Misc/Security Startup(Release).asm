...
+
		bne.w	Init_SkipPowerOn	; in case of a soft reset
...
Init_InputPSG:
		...
		jsr	(DetectEmulator).l
		move.b	d0,(Emulator_flag).w
		jsr	(PSGSilenceAll).l
...
-		move.l	d7,(a6)+
		...
		beq.s	+
		cmpi.l	#'BOSS',(Checksum_string).w		; has checksum routine already run?
		beq.s	GameInit						; if yes, branch
+		move.b	(HW_Version).l,d6
		andi.b	#$C0,d6
		move.b	d6,(Graphics_flags).w				; get region setting
		move.l	#'BOSS',(Checksum_string).w		; set flag so checksum won't run again
...
-	rept 32
	...
	endm
		...
		jsr	(SRAM_Load).l
		jsr	(Terminal_Screen).l
		move.b	#id_SeizureScreen,(Game_mode).w	; set Game Mode
...
GameModes:
ptr_GM_Seizure:		dc.l Seizure_Screen		; Seizure ($00)
ptr_GM_InsertCoin:	dc.l InsertCoin_Screen		; Insert Coin ($04)
ptr_GM_Options:		dc.l Options_Screen		; Options ($08)
ptr_GM_LevelSelect:	dc.l LevelSelect_Screen		; Level Select ($0C)
ptr_GM_Level:		dc.l GM_Level			; Level ($10)
ptr_GM_Ending:		dc.l Ending_Screen		; Ending ($14)
ptr_GM_Death:		dc.l Death_Screen			; Death ($18)